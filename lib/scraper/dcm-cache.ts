...
export async function getCachedDCMData(
  ...
  for (const row of cached) {
    result.set(row.trackingId, {
      ...
      plannedLatitude: row.plannedLatitude,
      plannedLongitude: row.plannedLongitude,
      ...
      concessionReason: row.concessionReason,
      address: row.address,
...
export async function cacheDCMData(data: DCMDeliveryData): Promise<void> {
  await prisma.dCMCache.upsert({
    ...
    create: {
      ...
      plannedLatitude: data.plannedLatitude,
      plannedLongitude: data.plannedLongitude,
